package net.avdw.git.changelog;

public class ResourceBundleKey {
    public static final String ADDED_TITLE = "added.title";
    public static final String CHANGED_TITLE = "changed.title";
    public static final String DEPRECATED_TITLE = "deprecated.title";
    public static final String FIXED_TITLE = "fixed.title";
    public static final String IGNORED_TITLE = "ignored.title";
    public static final String LINE_ITEM = "line.item";
    public static final String NO_CHANGE = "no.change";
    public static final String RELEASE_TITLE = "release.title(version,type,recommend,date)";
    public static final String REMOVED_TITLE = "removed.title";
    public static final String SECURITY_TITLE = "security.title";
    public static final String UNCLASSIFIED_TITLE = "unclassified.title";
}
